...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'send_email') {
        ...
        if (!$to || !$subject || !$body) { echo json_encode(['ok'=>false,'msg'=>'Missing required fields.']); exit; }
        $req = $db->prepare("SELECT r.*, a.name AS agent_name, u.email AS agent_email
            FROM requests r
            LEFT JOIN agents a ON a.id = r.agent_id
            LEFT JOIN users  u ON u.agent_id = r.agent_id
        ...
        if (!$from_email) { echo json_encode(['ok'=>false,'msg'=>'No email found for the agent linked to this request.']); exit; }
        $sent = send_hub_email($to, $subject, $body, $from_name, $from_email, $from_email);
        if ($sent) {
            log_email_note($db, $req_id, $cu['id'] ?? null, $subject, $body);
...
$tpl_by_cat = [];
foreach ($db->query("SELECT id, name, category FROM email_templates WHERE active=1 ORDER BY sort_order, name")->fetchAll(PDO::FETCH_ASSOC) as $t) {
    $tpl_by_cat[$t['category'] ?: 'General'][] = $t;
}
ksort($tpl_by_cat);

$today_ts  = mktime(0,0,0);
$pageTitle = 'Booked Requests';
$extra_css = '
...
<main>
  <div class="page-title">
    ...
    <a href="email_templates.php" class="btn btn-secondary btn-sm ml-auto">📧 Email Templates</a>
  </div>

  <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;margin-bottom:20px">
    <input type="text" id="searchBox" class="form-control" placeholder="Search customer, agent, destination…" style="max-width:280px">
  ...
  <div class="card">
    <div style="overflow-x:auto">
      <table class="data-table" id="bookedTable">
        <thead>
          <tr><th>Arrival</th><th>Departure</th><th>Customer</th><th>Email</th>
              <th>Agent</th><th>Destination</th><th style="text-align:center">Pax</th>
              <th style="text-align:center">Notes</th><th></th></tr>
        </thead>
        <tbody>
        <?php if (!$rows): ?>
        ...
        ?>
          <tr data-start="<?= $r['start_ts'] ?>"
            ...
            <td class="<?= $is_past ? 'date-past' : 'date-cell' ?>">
              <?= $r['start_date'] ? date('d M Y', strtotime($r['start_date'])) : '<span style="color:var(--red);font-size:.75rem">?</span>' ?>
            </td>
            <td style="font-size:.82rem"><?= $r['end_date'] ? date('d M Y', strtotime($r['end_date'])) : '—' ?></td>
  ...
  <div class="modal-box" style="max-width:860px">
    ...
    <div class="modal-body">
      ...
      <div class="tab-pane active" id="spane-edit" style="border:1.5px solid var(--grey-lt);border-top:none;border-radius:0 0 6px 6px">
        <textarea id="send_body" style="width:100%;min-height:260px;font-family:monospace;font-size:.78rem;border:none;padding:12px;resize:vertical;outline:none;border-radius:0 0 6px 6px"></textarea>
      </div>
      <div class="tab-pane" id="spane-preview">
        <div id="send_preview" style="border:1.5px solid var(--grey-lt);border-top:none;border-radius:0 0 6px 6px;padding:16px;min-height:260px;font-size:.85rem"></div>
      </div>
...
<script>
const TODAY_TS=<?= $today_ts ?>,INT_MAX=<?= PHP_INT_MAX ?>;
...
['sendOverlay','notesOverlay'].forEach(id=>{
  document.getElementById(id).addEventListener('click',e=>{if(e.target.id===id){closeSend();closeNotes();}});
});
...
function loadTemplate(){
  ...
  fetch('booked.php',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},
...
function doSend(){
  const btn=document.getElementById('btnSend'),alrt=document.getElementById('sendAlert');
  const body=new URLSearchParams({
    action:'send_email',request_id:document.getElementById('send_req_id').value,
    to:document.getElementById('send_to').value,subject:document.getElementById('send_subject').value,
    body:document.getElementById('send_body').value
  });
  btn.disabled=true;btn.textContent='Sending…';alrt.style.display='none';
...
function sendTab(tab,btn){
  ...
  if(tab==='preview'){document.getElementById('send_preview').innerHTML=document.getElementById('send_body').value||'<em style="color:var(--grey-mid)">Nothing to preview.</em>';}
}
...
function viewNotes(id,customer){
  ...
  fetch('booked.php',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:`action=get_notes&request_id=${id}`})
  .then(r=>r.json()).then(d=>{
    if(!d.ok||!d.notes.length){document.getElementById('notesBody').innerHTML='<p style="color:var(--grey-mid);text-align:center;padding:24px">No notes.</p>';return;}
    document.getElementById('notesBody').innerHTML=d.notes.map(n=>{
      ...
      return `<div class="note-card ${isEmail?'email-sent':''}">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:6px">
          <div><span class="chip ${isEmail?'chip-email':'chip-note'}">${isEmail?'📧 Email sent':'📝 Note'}</span>
            ${n.subject?`<span style="font-weight:600;font-size:.85rem;margin-left:8px">${esc(n.subject)}</span>`:''}
          </div>
          <span style="font-size:.72rem;color:var(--grey-mid);flex-shrink:0;margin-left:12px">${esc(n.created_at)}${n.user_name?' — '+esc(n.user_name):''}</span>
        </div>

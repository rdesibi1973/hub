...
$cu = current_user();
if (($cu['role_name'] ?? '') !== 'admin') { header('Location: dashboard.php'); exit; }
$db = db();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'save') {
        ...
        if ($id) {
            $db->prepare("UPDATE email_templates SET name=?,category=?,subject=?,body_html=?,active=?,sort_order=? WHERE id=?")
               ->execute([$name,$category,$subject,$body_html,$active,$sort_order,$id]);
        } else {
            $db->prepare("INSERT INTO email_templates (name,category,subject,body_html,active,sort_order) VALUES (?,?,?,?,?,?)")
               ->execute([$name,$category,$subject,$body_html,$active,$sort_order]);
        }
        echo json_encode(['ok'=>true]); exit;
    }
    if ($action === 'delete') {
        $db->prepare("DELETE FROM email_templates WHERE id=?")->execute([(int)$_POST['id']]);
        echo json_encode(['ok'=>true]); exit;
    }
    if ($action === 'get') {
    ...
    if ($action === 'toggle_active') {
        $db->prepare("UPDATE email_templates SET active=1-active WHERE id=?")->execute([(int)$_POST['id']]);
        echo json_encode(['ok'=>true]); exit;
    }
...
// ── Data ──────────────────────────────────────────────────────────────────────
$templates  = $db->query("SELECT * FROM email_templates ORDER BY sort_order ASC, name ASC")->fetchAll(PDO::FETCH_ASSOC);
...
.tpl-two-col{display:grid;grid-template-columns:1fr 320px;gap:24px}
.var-panel{background:var(--off-white);border-radius:8px;border:1px solid var(--grey-lt);padding:16px;height:fit-content}
.var-panel h4{font-size:.72rem;font-weight:700;text-transform:uppercase;letter-spacing:.1em;color:var(--grey-dk);margin-bottom:10px}
...
.var-desc{color:var(--grey-mid);font-size:.74rem}
  ...
  <div class="card">
    <div style="overflow-x:auto">
      <table class="data-table">
        <thead>
          <tr><th>#</th><th>Name</th><th>Category</th><th>Subject</th>
              <th style="text-align:center">Active</th><th style="text-align:center">Order</th><th></th></tr>
        </thead>
        <tbody>
        <?php if (!$templates): ?>
        ...
        <?php foreach ($templates as $t): ?>
          <tr style="<?= $t['active'] ? '' : 'color:var(--grey-mid)' ?>">
            ...
            <td style="font-size:.78rem;max-width:300px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap"><?= e($t['subject']) ?></td>
    ...
    <div class="tpl-modal-body">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr 140px 100px;gap:16px;margin-bottom:20px">
        ...
        <div style="padding-top:28px">
          <label class="form-check"><input type="checkbox" id="f_active" checked><span>Active</span></label>
        </div>
      </div>
      ...
      <div class="tpl-two-col">
        <div>
          ...
          <div class="tab-pane" id="pane-preview">
            <div id="previewContent" style="border:1.5px solid var(--grey-lt);border-top:none;border-radius:0 0 6px 6px;padding:16px;min-height:300px;font-size:.85rem"></div>
          </div>
        </div>
        <div class="var-panel">
          <h4>Available Variables</h4>
          <p style="font-size:.74rem;color:var(--grey-mid);margin-bottom:10px">Click to insert at cursor:</p>
          <?php foreach ([
            '{{customer_name}}'=>'Customer name','{{destination}}'=>'Destination',
            '{{period}}'=>'Period (text)','{{pax}}'=>'Number of pax',
            '{{start_date}}'=>'Arrival date','{{end_date}}'=>'Departure date',
...
function openModal(id) {
  ...
  if (id) {
    fetch('email_templates.php',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:`action=get&id=${id}`})
...
document.getElementById('tplOverlay').addEventListener('click',e=>{if(e.target.id==='tplOverlay')closeModal();});

function saveTemplate(){
  const al=document.getElementById('saveAlert');
  const body=new URLSearchParams({
    action:'save',id:document.getElementById('f_id').value,
    name:document.getElementById('f_name').value,category:document.getElementById('f_category').value,
    subject:document.getElementById('f_subject').value,body_html:document.getElementById('f_body_html').value,
    sort_order:document.getElementById('f_sort_order').value,active:document.getElementById('f_active').checked?'1':''
  });
  fetch('email_templates.php',{method:'POST',body}).then(r=>r.json()).then(d=>{
    if(d.ok){location.reload();}
    else{al.style.cssText='display:block;background:var(--red-lt);color:var(--red-dk);';al.textContent=d.msg;}
  });
}
function deleteTpl(id,name){
  if(!confirm(`Delete template "${name}"?`))return;
  fetch('email_templates.php',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:`action=delete&id=${id}`})
  .then(r=>r.json()).then(d=>{if(d.ok)location.reload();});
}
function toggleActive(id){
  fetch('email_templates.php',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:`action=toggle_active&id=${id}`})
...
function switchTab(tab,btn){
  ...
  if(tab==='preview'){document.getElementById('previewContent').innerHTML=document.getElementById('f_body_html').value||'<em style="color:var(--grey-mid)">Nothing to preview.</em>';}
}
</script>

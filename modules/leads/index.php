...
<div class="tab-nav">
  ...
  <button class="tab-btn"           onclick="switchTab('audit',this)">🔍 Audit Excel</button>
  <button class="tab-btn"           onclick="switchTab('conflicts',this)">🔀 Fix Conflicts</button>
</div>

<!-- ═══ TAB 1: DAILY MOVEMENTS ═══ -->
<div id="tab-movements" class="tab-panel active">
  <div class="ctrl-bar">
...
function copyWADate(date, btn){
  ...
  const text=buildWAForDate(rows);
  navigator.clipboard.writeText(text).then(()=>{
    if(btn){const orig=btn.textContent;btn.textContent='✓ Copied!';btn.style.background='var(--green)';btn.style.color='var(--white)';setTimeout(()=>{btn.textContent=orig;btn.style.background='';btn.style.color='';},2000);}
  }).catch(()=>{
    // fallback
...
function copyExtModal(){const ta=document.getElementById('extModalText');ta.select();let ok=false;try{ok=document.execCommand('copy');}catch(e){}if(!ok&&navigator.clipboard)navigator.clipboard.writeText(ta.value).catch(()=>{});const btn=document.getElementById('extModalCopyBtn');btn.textContent='Copied!';btn.classList.add('copied');setTimeout(()=>{btn.textContent='Copy to clipboard';btn.classList.remove('copied');},2000);}
...
function copyGridAllRows(){
  ...
  navigator.clipboard.writeText(tsv).then(()=>{const btn=document.getElementById('copyGridAllBtn');if(btn){btn.textContent='✓ Copied!';setTimeout(()=>btn.textContent='📋 Copy all rows',2000);}}).catch(()=>{const ta=document.createElement('textarea');ta.value=tsv;ta.style.position='fixed';ta.style.opacity='0';document.body.appendChild(ta);ta.select();document.execCommand('copy');document.body.removeChild(ta);const btn=document.getElementById('copyGridAllBtn');if(btn){btn.textContent='✓ Copied!';setTimeout(()=>btn.textContent='📋 Copy all rows',2000);}});
}
...
function cfLoadFiles(files){
  ...
  const promises=Array.from(files).map(f=>new Promise((res,rej)=>{
    const reader=new FileReader();
    reader.onload=e=>{try{const wb=XLSX.read(e.target.result,{type:'array',cellDates:true});const sname=wb.SheetNames.find(n=>/CONF/i.test(n))||wb.SheetNames.find(n=>/RECAP/i.test(n))||wb.SheetNames[wb.SheetNames.length-1];res({filename:f.name,wb,sname,file:f});}catch(err){rej(f.name+': '+err.message);}};
...
function cfApply(discs){
  const base=cfWBs[cfBaseIdx];const wb=XLSX.utils.book_new();const sheet=Object.assign({},base.wb.Sheets[base.sname]);
  discs.forEach((d,i)=>{const val=document.getElementById('cfs-'+i)?.value??'';const addr=XLSX.utils.encode_cell({r:d.row,c:d.col});if(val==='')delete sheet[addr];else sheet[addr]=sheet[addr]?Object.assign({},sheet[addr],{v:val,w:val}):{v:val,t:'s'};});
  XLSX.utils.book_append_sheet(wb,sheet,base.sname);
  const stem=base.filename.replace(/\s*\([^)]*conflicted copy[^)]*\)/i,'').replace(/\.xlsx$/i,'').trim();
  XLSX.writeFile(wb,stem+'_FIXED.xlsx');
}

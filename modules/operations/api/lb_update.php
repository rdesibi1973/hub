...
try {
    $stmt = $pdo->prepare(
        'UPDATE lunch_boxes
         SET client_name=?, safari_date=?, travelers=?, jeeps=?, guide=?, box_type=?,
             extra_details=?, folder_name=?, notes=?
         WHERE id=?'
    );
    $stmt->execute([$clientName, $dateSql, $travelers, $jeeps, $guide, $boxType, $extraDetails, $folderName, $notes, $id]);

...
$mode = $body['mode'] ?? ''; // 'traveler' or 'group'

try {
    if ($mode === 'traveler') {
        $id = (int)($body['id'] ?? 0);
        if (!$id) { echo json_encode(['error' => 'Missing id']); exit; }
        $pdo->prepare('DELETE FROM medivac_travelers WHERE id = ?')->execute([$id]);
        ...
        $stmt = $pdo->prepare('DELETE FROM medivac_travelers WHERE group_ref = ?');
        $stmt->execute([$ref]);

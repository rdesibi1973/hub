...
// ── PROGRAMS ──────────────────────────────────────────────────────────────────
function iti_get_programs(string $type = null, string $status = null): array {
    $where  = [];
    $params = [];
    if ($type)   { $where[] = 'p.program_type = ?'; $params[] = $type; }
...
// ── REQUESTS ──────────────────────────────────────────────────────────────────
function iti_get_requests(string $status = null): array {
    $where  = $status ? 'WHERE status = ?' : '';
    $params = $status ? [$status] : [];
    $st = db()->prepare("SELECT * FROM iti_requests {$where} ORDER BY created_at DESC");
    $st->execute($params);
    return $st->fetchAll();
}

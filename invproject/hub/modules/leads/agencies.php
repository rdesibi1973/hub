...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'edit') {
        ...
        if ($nome === '') {
            ...
            $db->prepare('UPDATE agencies SET nome=?, short_name=?, attiva=? WHERE id=?')
               ->execute([$nome, $short ?: null, $attiva, $id]);
      ...
      <tr id="row-<?= $ag['id'] ?>">
        ...
        <td class="edit-<?= $ag['id'] ?>" style="display:none" colspan="3">
          <form method="POST" style="display:flex; gap:8px; align-items:center;">
            ...
            <input type="text" name="short_name" value="<?= h($ag['short_name'] ?? '') ?>" style="width:160px;padding:5px 8px;border:1.5px solid var(--grey-lt);border-radius:5px;font-size:.85rem;">
